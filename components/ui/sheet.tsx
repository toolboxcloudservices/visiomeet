...
import * as SheetPrimitive from '@radix-ui/react-dialog'; // Radix UI dialog primitives for building modal-like sheets
import { cva, type VariantProps } from 'class-variance-authority'; // Utility for creating class name variants
...
// Component for closing the Sheet
const SheetClose = SheetPrimitive.Close;

// Portal component for rendering the Sheet outside the main DOM hierarchy
...
// Variants for styling the Sheet's content based on its side placement
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-white p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out dark:bg-slate-950',
  {
    ...
    defaultVariants: {
      side: 'right', // Default placement is on the right side
    },
  }
);
...
>(({ side = 'right', className, children, ...props }, ref) => (
  <SheetPortal>
    <SheetOverlay /> {/* Overlay for the Sheet */}
    ...
    >
      ...
      >
        <X className="size-4 text-white" /> {/* Close icon */}
